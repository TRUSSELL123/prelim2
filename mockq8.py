        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    if MenuChoice == "P" and not self.__PeekUsed:
                        for i in range(3):
    ...
    def __GetCardFromDeck(self, CardChoice):
        ...
        while self.__Hand.GetNumberOfCards() < 5 and self.__Deck.GetNumberOfCards() > 0:
            if self.__Deck.GetCardDescriptionAt(0) == "Gen":
                MenuChoice = False
                while type(MenuChoice) != int:
                    MenuChoice = input("Which challendge would you like solved?")
                self.__CurrentLock.SetChallengeMet(MenuChoice, True)
            elif self.__Deck.GetCardDescriptionAt(0) == "Dif":
