...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print(f"There are {self.__Deck.GetNumberOfCards()} cards left")
    ...
    def Mulligan(self):
        ...
        for i in self.__Sequence.GetAllCards():
            self.__Deck.AddCard(i)
            self.__Sequence.RemoveCard(i.GetCardNumber())
        for i in self.__Discard.GetAllCards():
            if i.GetDescription() != "Dif":
                self.__Deck.AddCard(i)
            self.__Discard.RemoveCard(i.GetCardNumber())
        for i in self.__Hand.GetAllCards():
            self.__Deck.AddCard(i)
            self.__Hand.RemoveCard(i.GetCardNumber())
        self.__Deck.Shuffle()
